...
class ProjectGuidanceTests(unittest.TestCase):
    def test_guidance_defines_the_unattended_refresh_safety_contract(self) -> None:
        ...
        required_phrases = (
            ...
            "Blizzard-only",
            "all required locales must pass",
            "Optional `esMX` does not block a release",
    ...
    def test_guidance_defines_the_complete_language_and_locale_policy(
        ...
        required_phrases = (
            ...
            "Require `esES` as the canonical Spanish",
            "`esMX` as an optional regional override",
            "stored `esES` text before falling back to English",
            "Do not store a duplicate `esES` localization under an `esMX` key",
